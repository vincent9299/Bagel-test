...
torchrun --nnodes=1 --node_rank=0 --nproc_per_node=$NGPU \
  ...
  --save_every 100000 \
  --warmup_steps 20 \
  --results_dir /root/data/results/lora_overfit_edit \
  --checkpoint_dir /root/data/results/lora_overfit_edit/checkpoints \
  --wandb_project bagel-lora \
  --wandb_name lora_overfit_edit \
  --wandb_offline True \
  --lora_r $LORA_R \
  --lora_alpha $LORA_R \
  --example_data_dir /root/data/bagel_example_overfit \
  "$@"
