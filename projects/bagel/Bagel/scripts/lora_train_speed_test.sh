...
torchrun --nnodes=1 --node_rank=0 --nproc_per_node=$NGPU \
  ...
  --save_every 100000 \
  --warmup_steps 100 \
  --results_dir /root/data/projects/bagel/results/lora_flash_speed \
  --checkpoint_dir /root/data/projects/bagel/results/lora_flash_speed/checkpoints \
  --wandb_project bagel-lora \
  --wandb_name lora_flash_speed \
  --wandb_offline True \
  --lora_r $LORA_R \
  --lora_alpha $LORA_R \
  --example_data_dir /root/data/projects/bagel/bagel_example \
  "$@"
